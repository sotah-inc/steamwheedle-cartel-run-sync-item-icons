...
var state run.SyncItemIconsState
...
func init() {
	...
	// producing sync-item-icons state
	logging.WithFields(logrus.Fields{
		"project":      projectId,
		"service-name": serviceName,
		"port":         port,
	}).Info("Producing sync-item-icons state")

	state, err = run.NewSyncItemIconsState(run.SyncItemIconsStateConfig{ProjectId: projectId})
	if err != nil {
		log.Fatalf("Failed to generate state: %s", err.Error())
